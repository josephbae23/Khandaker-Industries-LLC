    ...
    <section className="section-py bg-sand-50 relative overflow-hidden">
      ...
      <div className="container-main relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
          ...
          <p className={`text-navy-600/70 text-base max-w-md leading-relaxed ${isRtl ? "text-right" : "text-left"}`}>
            {t.subtitle}
          </p>
        </div>
          ...
          {t.items.map((item, i) => (
            ...
            >
              ...
              <div className="p-6 relative">
                {/* Corner accent */}
                {isRtl ? (
                  <div className="absolute top-0 left-0 w-0 h-0 border-r-[40px] border-r-transparent border-t-[40px] border-t-gold-400/10 group-hover:border-t-gold-400/30 transition-colors" />
                ) : (
                ...
                <h3 className={`text-navy-800 font-semibold text-lg mb-2 ${isRtl ? "text-right" : "text-left"}`}>{item.title}</h3>
                <p className={`text-navy-600/60 text-sm leading-relaxed relative z-10 ${isRtl ? "text-right" : "text-left"}`}>{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
